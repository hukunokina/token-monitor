...
test('derived KNOWN_CLIENTS keeps the established id order', () => {
  ...
  assert.equal(
    KNOWN_CLIENTS,
    'claude,codex,opencode,hermes,openclaw,cursor,antigravity,cline,amp,droid,kimi,qwen,grok,copilot,pi,zed,kilo,commandcode,micode,zcode,kiro,codebuddy,workbuddy,proma,qodercn,reasonix,dsh,cherrystudio,lmstudio,unsloth,muse'
  );
});
...
test('derived DEFAULT_CLIENTS keeps the existing default-tracked CSV', () => {
  ...
  assert.equal(
    DEFAULT_CLIENTS,
    'claude,codex,opencode,hermes,openclaw,cursor,antigravity,cline,amp,droid,kimi,qwen,grok,copilot,pi,zed,kilo,commandcode,zcode,kiro,codebuddy,workbuddy,proma,reasonix,dsh,cherrystudio,lmstudio,unsloth,muse'
  );
});

test('derived PARSE_LOCAL_CLIENTS still lists exactly the local adapters', () => {
  assert.deepEqual([...PARSE_LOCAL_CLIENTS], ['proma', 'qodercn', 'muse']);
  assert.deepEqual([...LOCALLY_PARSED_CLIENT_IDS], ['proma', 'qodercn', 'muse']);
});
